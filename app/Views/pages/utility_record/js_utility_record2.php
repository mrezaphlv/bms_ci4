...
$(document).ready(function() {
    ...
    $('#btnSaveUtilityRecord').click(function(){
       saveNewUR();
    });

    $('#btnUpdateUtilityRecord').click(function(){
       updateUR();
    });
    ...
    //     });
        ...
        function saveNewUR(data) {
            ...
            $.ajax({
                ...
                url: '<?php echo base_url(); ?>utility_record/saveNewUR/',
                success: function(response) {
                    ...
                    if (response) {
                        ...
                        window.location = '<?php echo base_url(); ?>utility_record';
                    } else {
                        Swal.fire({
                            icon: 'error',
                            title: 'Oops...',
        ...
        function updateUR(data) {
            ...
            $.ajax({
                ...
                processData: false,  // Important!
                contentType: false,  // Important!
                url: '<?php echo base_url(); ?>utility_record/updateData2/',
                success: function(response) {
                    console.log(response);
                    if (response) {
                        Swal.fire({
    ...
    //         var id_unit = $('#id_unit').val();
    //         var id_utilities = $('#id_utilities').val();
    //         var namafile = $('#foto_file').val();
    //         var periode = $('#periode').val();
    //         var end_meter = $('#end_meter').val();
    //         var id_meter = $('#id_meter').val();
    //         var ids = $('#ids').val();
    //         console.log(id_unit, id_utilities, id_meter, periode, end_meter, namafile, ids);
    //         $.ajax({
    //             type: "POST",
    //             data: {
    //                 id_unit: id_unit,
    ...
    //     }
      ...
      function saveNewUR(data) {
            ...
            formData.append('end_meter', end_meter);
            formData.append('id_meter', id_meter);
            formData.append('kode_meter', kode_meter);
            formData.append('start_meter', start_meter);
            formData.append('ids', ids);
            formData.append('foto_file', $('#foto_file')[0].files[0]); // Ambil file
            $.ajax({
...
function gantiMeterId() {
    ...
    if(id_unit != '' && id_utilities != ''){
        ...
        dataType: "JSON",
        url: '<?php echo base_url(); ?>utility_record/cariUtilities/',
    	beforeSend: function() {show_load();},
		complete: function() {hide_load();},
        success: function(response) {
