...
class Hand_over extends MyController
{
    protected $db;
    protected $session;
    ...
    public function index()
    {
        ...
        if ($this->akses->can_view == 1) {
            return $this->template('pages/hand_over/vhand_over', [
                'akses' => $this->akses,
            ]);
        }
    }
    ...
    public function grid()
    {
        ...
        $rows   = max(1, (int) ($post['rows'] ?? ($post['length'] ?? 20)));
        $offset = isset($post['start']) ? (int) $post['start'] : (($page - 1) * $rows);
        ...
        $sort  = (string) ($post['sort'] ?? 'id');
        $order = (string) ($post['order'] ?? 'desc');

        if (isset($post['order'][0]['column'])) {
            $indexColOrder = $post['order'][0]['column'];
            $sort = (string) ($post['columns'][$indexColOrder]['field'] ?? $post['columns'][$indexColOrder]['data'] ?? 'id');
            $order = (string) ($post['order'][0]['dir'] ?? 'desc');
        }

        $result = $this->mhand_over->grid([
            'offset' => $offset,
            'limit' => $rows,
            'sort' => $sort,
            'order' => $order,
        ...
        if (!empty($this->request->getPost('tgl_lunas')) && !empty($this->request->getPost('tgl_bast'))) {
            ...
            if (!empty($this->request->getPost('idp_undangan')) && !empty($this->request->getPost('idp_bast'))) {
                ...
                if ($nomer_bast) {
                    ...
                    $upd = array(
                        'no_agreement' => $nomer_bast->getRow()->nomer,
                        'tgl_lunas' => $tgl_bast,
